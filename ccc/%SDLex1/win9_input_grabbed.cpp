...
int main(int argc, char *argv[]){
    SDL_Window      *pWindowA = NULL;
    SDL_Window      *pWindowB = NULL;
    ...
    //创建窗口
    pWindowA = SDL_CreateWindow("exampleA",100,100,400,400,SDL_WINDOW_INPUT_FOCUS);
    if (NULL == pWindowA)    return 0;
    
    pWindowB = SDL_CreateWindow("exampleB",300,300,400,400,SDL_WINDOW_MOUSE_FOCUS);
    if (NULL == pWindowB)    return 0;

    //创建Renderer
    pRenderer = SDL_CreateRenderer(pWindowA, -1, 0);
    ...
    while (!isWindowToBeClosed) {
        while (SDL_PollEvent(&gEvent)) {
            ...
            /// 键盘操作
            case SDL_MOUSEWHEEL:{
                PrintText("SDL_MOUSEWHEEL", gEvent.text.text);
