...
BEGIN_MESSAGE_MAP(SettingsDlg, CDialog)
	...
	ON_NOTIFY(NM_CLICK, IDC_SYSLINK_DTMF_METHOD, &SettingsDlg::OnNMClickSyslinkDTMFMethod)
	...
	ON_NOTIFY(NM_CLICK, IDC_SYSLINK_STUN_SERVER, &SettingsDlg::OnNMClickSyslinkStunServer)
	...
	ON_NOTIFY(NM_CLICK, IDC_SYSLINK_DISABLE_VP8, &SettingsDlg::OnNMClickSyslinkDisableVP8)
	...
	ON_NOTIFY(NM_CLICK, IDC_SYSLINK_CRASH_REPORT, &SettingsDlg::OnNMClickSyslinkCrashReport)
#ifdef _GLOBAL_VIDEO
...
void SettingsDlg::OnNMClickSyslinkDisableVP8(NMHDR *pNMHDR, LRESULT *pResult)
{
	OpenHelp(_T("disableVP8"));
	*pResult = 0;
}
