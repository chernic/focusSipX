...
PJ_DEF(void) pjsua_call_vid_strm_op_param_default(pjsua_call_vid_strm_op_param *param){
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_dev_get_info(pjmedia_vid_dev_index id, pjmedia_vid_dev_info *vdi){
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_dev_set_setting( pjmedia_vid_dev_index id, pjmedia_vid_dev_cap cap, const void *pval, pj_bool_t keep){
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_dev_get_setting( pjmedia_vid_dev_index id, pjmedia_vid_dev_cap cap, void *pval){
    pj_status_t status = PJ_SUCCESS;
    pjsua_vid_win_id wid = vid_preview_get_win(id, PJ_FALSE);
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_enum_devs(pjmedia_vid_dev_info info[], unsigned *count){
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_enum_codecs( pjsua_codec_info id[], unsigned *p_count ){
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_codec_set_priority( const pj_str_t *codec_id, pj_uint8_t priority ){
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_codec_get_param(const pj_str_t *codec_id, pjmedia_vid_codec_param *param){
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_codec_set_param(const pj_str_t *codec_id, const pjmedia_vid_codec_param *param){
    const pjmedia_vid_codec_info *info[2];
    unsigned count = 2;
    pj_status_t status;
...
static pj_status_t  create_vid_window(
    ...
    if (type == PJSUA_WND_TYPE_PREVIEW) {
        ...
        //wid = vid_preview_get_win(rend_id, cap_id, PJ_FALSE);
...
static pj_status_t       create_vid_win(pjsua_vid_win_type type,const pjmedia_format *fmt,pjmedia_vid_dev_index rend_id,pjmedia_vid_dev_index cap_id,pj_bool_t show,unsigned wnd_flags,const pjmedia_vid_dev_hwnd  *wnd,pjsua_vid_win_id *id){
    ...
    if ( type == PJSUA_WND_TYPE_PREVIEW ) {
        ...
        if (wid != PJSUA_INVALID_ID) {
            ...
            PJ_LOG(4,(THIS_FILE, "Window already exists for cap_dev=%d, returning wid=%d", cap_id, wid));
            ...
            status = pjmedia_vid_dev_stream_set_cap(strm, PJMEDIA_VID_DEV_CAP_OUTPUT_HIDE, &hide);
            if (status != PJ_SUCCESS) {
            ...
            status = pjmedia_vid_dev_stream_set_cap(strm, PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW_FLAGS, &wnd_flags);
            if (status != PJ_SUCCESS) {
                PJ_PERROR(4,(THIS_FILE, status, "Ignored error on setting window flags on wid=%d", wid));
            }

            if (wnd) {
                status = pjmedia_vid_dev_stream_set_cap( strm, PJMEDIA_VID_DEV_CAP_OUTPUT_WINDOW, wnd);
                if (status != PJ_SUCCESS) {
                    PJ_PERROR(4, (THIS_FILE, status,"Ignored error on setting window handle on wid=%d", wid));
                }
            }
    ...
    if (w->type == PJSUA_WND_TYPE_PREVIEW) {
        ...
        if (enable_native_preview &&(vdi.caps & PJMEDIA_VID_DEV_CAP_INPUT_PREVIEW)) {
            // ABChecked
            /* Device supports native preview! */
            w->is_native = PJ_TRUE;
        }

        status = pjmedia_vid_dev_default_param(w->pool, cap_id, &vp_param.vidparam);
        if (status != PJ_SUCCESS) {
            goto on_error;
        }
        ...
        /* Create video tee */
        status = pjmedia_vid_tee_create(w->pool, fmt, VID_TEE_MAX_PORT, &w->tee);
        if (status != PJ_SUCCESS)
            goto on_error;
    ...
    if (!w->is_native) {
        status = pjmedia_vid_dev_default_param(w->pool, rend_id, &vp_param.vidparam);
        if (status != PJ_SUCCESS){
            goto on_error;
        }
...
static void free_vid_win(pjsua_vid_win_id wid){
   ...
   // if (w->vp_cap   && pjmedia_vid_port_is_running(w->vp_cap)) 
    if ( w->vp_cap ) {
...
////////////
// E20180312-0900-pjsua_call_hangup
////////////
    //if ( w->vp_rend && pjmedia_vid_port_is_running(w->vp_rend) ) {
    if ( w->vp_rend ) {
        pjmedia_event_unsubscribe(NULL, &call_media_on_event, NULL, w->vp_rend);
        pjmedia_vid_port_stop      (w->vp_rend);
		// S20180316-1410-pjsua_call_hangup
		// vp_rend 不用 disconnect vid_port 否则报错
...
///
        ...
        if (si->dir & PJMEDIA_DIR_ENCODING && !call->local_hold){
            ...
            acc_enc = &pjsua_var.acc[cm->call->acc_id];
...
///
    ...
    if (!acc->cfg.vid_out_auto_transmit && cm->strm.v.stream) {
        status = pjmedia_vid_stream_pause(cm->strm.v.stream,
...
/// 2018-01-03
    if(!w && NULL != w->tee){
        pjmedia_vid_port_stop( (pjmedia_vid_port *) w->tee );
    }
    ...
    if (status == PJ_SUCCESS) {
        ...
        /* Unsubscribe event */
    pjmedia_event_unsubscribe(NULL, &call_media_on_event, call_med, w->vp_cap);
    ...
    pjmedia_vid_port_stop(w->vp_rend);
    pjmedia_event_unsubscribe(NULL, &call_media_on_event, call_med, w->vp_rend);
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_enum_wins   ( pjsua_vid_win_id wids[], unsigned *count){
    unsigned i, cnt;
...
PJ_DEF(pj_status_t) pjsua_vid_win_set_show( pjsua_vid_win_id wid, pj_bool_t show){
    ...
    hide = !show;
    status = pjmedia_vid_dev_stream_set_cap(s, PJMEDIA_VID_DEV_CAP_OUTPUT_HIDE, &hide);
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_win_set_dir ( pjsua_vid_win_id wid, const pj_str_t *dir){
    pjsua_vid_win *w;
    pjmedia_vid_dev_stream *s;
    pj_status_t status;
    ...
    status = pjmedia_vid_dev_stream_set_dir(s, dir);
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_win_set_pos ( pjsua_vid_win_id wid, const pjmedia_coord *pos){
    pjsua_vid_win *w;
    pjmedia_vid_dev_stream *s;
    pj_status_t status;
    ...
    status = pjmedia_vid_dev_stream_set_cap(s, PJMEDIA_VID_DEV_CAP_OUTPUT_POSITION, pos);
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_win_set_size( pjsua_vid_win_id wid, const pjmedia_rect_size *size){
    pjsua_vid_win *w;
    pjmedia_vid_dev_stream *s;
    pj_status_t status;
    ...
    status = pjmedia_vid_dev_stream_set_cap(s,PJMEDIA_VID_DEV_CAP_OUTPUT_RESIZE, size);

    PJSUA_UNLOCK();

    return status;
}
PJ_DEF(pj_status_t) pjsua_vid_win_set_button_size( pjsua_vid_win_id wid, const pjmedia_rect_size *size){
    pjsua_vid_win *w;
    pjmedia_vid_dev_stream *s;
    pj_status_t status;
    ...
    // pjsua_vid_win_set_pos->pjmedia_vid_dev_stream_set_cap->set_cap(PJMEDIA_VID_DEV_CAP_BUTTON_SIZE)
    status = pjmedia_vid_dev_stream_set_cap(s,PJMEDIA_VID_DEV_CAP_BUTTON_SIZE, size);
...
/// via : videodev.h
PJ_DEF(pj_status_t) pjsua_vid_win_set_previw_pos ( pjsua_vid_win_id wid, const pjmedia_coord *pos){
    pjsua_vid_win *w;
    pjmedia_vid_dev_stream *s;
    pj_status_t status;
...
PJ_DEF(pj_status_t) pjsua_vid_win_set_previw_size( pjsua_vid_win_id wid, const pjmedia_rect_size *size){
    pjsua_vid_win *w;
    pjmedia_vid_dev_stream *s;
    pj_status_t status;
...
PJ_DEF(pj_status_t) pjsua_vid_win_set_stream_pos ( pjsua_vid_win_id wid, const pjmedia_coord *pos){
    pjsua_vid_win *w;
    pjmedia_vid_dev_stream *s;
    pj_status_t status;
    ...
    // pjsua_vid_win_set_pos->pjmedia_vid_dev_stream_set_cap->set_cap(PJMEDIA_VID_DEV_CAP_STREAM_POS)
    status = pjmedia_vid_dev_stream_set_cap(s,PJMEDIA_VID_DEV_CAP_STREAM_POS, pos);

    PJSUA_UNLOCK();

    return status;
}
PJ_DEF(pj_status_t) pjsua_vid_win_set_stream_size( pjsua_vid_win_id wid, const pjmedia_rect_size *size){
    pjsua_vid_win *w;
    pjmedia_vid_dev_stream *s;
    pj_status_t status;
    ...
    // pjsua_vid_win_set_pos->pjmedia_vid_dev_stream_set_cap->set_cap(PJMEDIA_VID_DEV_CAP_STREAM_SIZE)
    status = pjmedia_vid_dev_stream_set_cap(s,PJMEDIA_VID_DEV_CAP_STREAM_SIZE, size);
 ...
 *
 */
PJ_DEF(pj_status_t) pjsua_vid_win_set_win ( pjsua_vid_win_id wid, const pjmedia_vid_dev_hwnd *win){
    pjsua_vid_win *w;
    pjmedia_vid_dev_stream *s;
    pj_status_t status;
...
static void call_get_vid_strm_info(pjsua_call *call,int *first_active,int *first_inactive,unsigned *active_cnt,unsigned *cnt){
...
/* Send SDP reoffer. */
static pj_status_t call_reoffer_sdp(pjsua_call_id call_id,const pjmedia_sdp_session *sdp){
    pjsua_call *call;
    pjsip_tx_data *tdata;
    pjsip_dialog *dlg;
    pj_status_t status;
...
static pj_status_t call_add_video(pjsua_call *call,pjmedia_vid_dev_index cap_dev,pjmedia_dir dir){
 ...
 * remove/disable.
 */
static pj_status_t call_modify_video(pjsua_call *call,int med_idx,pjmedia_dir dir,pj_bool_t remove){
    pjsua_call_media *call_med;
    const pjmedia_sdp_session *current_sdp;
    pjmedia_sdp_session *sdp;
    pj_status_t status;
...
static pj_status_t call_change_cap_dev(pjsua_call *call, int med_idx, pjmedia_vid_dev_index cap_dev){
    pjsua_call_media *call_med;
...
#if ENABLE_EVENT
    pjmedia_event_subscribe(NULL, &call_media_on_event, call_med, new_w->vp_cap);
#endif
...
on_error:
    ...
    pjmedia_event_unsubscribe(NULL, &call_media_on_event, call_med, new_w->vp_cap);
...
static pj_status_t call_set_tx_video(pjsua_call *call,int med_idx,pj_bool_t enable){
    pjsua_call_media *call_med;
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t) pjsua_call_set_vid_strm (pjsua_call_id call_id,pjsua_call_vid_strm_op op,const pjsua_call_vid_strm_op_param *param){
    pjsua_call *call;
 ...
 */
PJ_DEF(pj_bool_t) pjsua_call_vid_stream_is_running( pjsua_call_id call_id,int med_idx,pjmedia_dir dir){
    pjsua_call *call;
    pjsua_call_media *call_med;
