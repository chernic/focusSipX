...
#define THIS_FILE   "focusua_common.c"
...
//#endif
    ...
    */
    ...
    int mypjsua_test_config (pjsua_config *cfg, char* buf, pj_size_t bufsize)
    {
        // Ref : os_error_linux_kernel.c(61)
        PJ_UNUSED_ARG(cfg);
        PJ_UNUSED_ARG(bufsize);
        // strcpy(errmsg, "Here's no test function to run.");
        ...
        if (cfg==NULL)
            strcpy(errmsg, "pjsua_config is NULL");
        else
            snprintf(errmsg, sizeof(errmsg), "pjsua_config is OK");
...
void vid_print_dev(int id, const pjmedia_vid_dev_info *vdi, const char *title)
{
    ...
    // CC
    // PJ_ LOG(3,(THIS_FILE, "%3d %s [%s][%s] %s", id, vdi->name, vdi->driver,
          // dirname, title));
    // PJ_ LOG(3,(THIS_FILE, "    Supported capabilities: %s", capnames));
    // PJ_ LOG(3,(THIS_FILE, "    Supported formats: %s%s", formats,
                  // (st_len<0? " ..." : "")));
...
void vid_list_devs()
{
    ...
    pj_status_t status;
    // CC
    // PJ_ LOG(3,(THIS_FILE, "Video device list:"));
    
    count = pjsua_vid_dev_count();
    if (count == 0) {
    // CC
    // PJ_ LOG(3,(THIS_FILE, " - no device detected -"));
    
    return;
    } else {
    // CC
    // PJ_ LOG(3,(THIS_FILE, "%d device(s) detected:", count));
...
// {
    ...
    // pj_log_set_decor(log_decor & ~(PJ_ LOG_HAS_NEWLINE | PJ_ LOG_HAS_CR));
// / CC
    // PJ_ LOG(3,(THIS_FILE, "\n"));
// 
    // pj_log_set_decor(0);
