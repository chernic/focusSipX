...
struct sdl_stream
{
    ...
    // ABCherniccccccc
    char                    sBuf[32];
    SDL_Window              *Perwindow;     /**< Display window.    */
...
static pj_status_t handle_event(void *data)
{
    ...
    switch(sevent.type) {
        ...
        int isToolBarShowed;
        int isChecked;
        int isWindowToBeClosed;
        strm->pToolBar->Handle_event              (sevent, &isToolBarShowed);
        strm->pToolBar->mButtonPlay.Handle_event  (sevent, &isChecked);
        strm->pToolBar->mButtonClose.Handle_event (sevent, &isWindowToBeClosed);
...
#endif /* PJMEDIA_VIDEO_DEV_SDL_HAS_OPENGL */
    ///////////////////////// ABChernic scr_texS
    if (strm->scr_texS) {                                                       // destroy<<
        SDL_DestroyTexture(strm->scr_texS);                                     // destroy
        strm->scr_texS = NULL;                                                  // destroy>>
    }
    ///////////////////////// ABChernic scr_texP
    if (strm->scr_texP) {                                                       // destroy<<
        SDL_DestroyTexture(strm->scr_texP);                                     // destroy
        strm->scr_texP = NULL;                                                  // destroy>>
    }
                     ...
                     pjmedia_vid_dev_hwnd *hwnd)
{
    ...
    if (use_app_win) {
        ...
        // http://t.cn/RTqltu8
        // ABCherniccccccccccccccccccccccccccccccc
        //   if (SDL_VideoInit(NULL) != 0) {
        //  sdl_log_err( "SDL_VideoInit() error: " );
        //  sdl_log_err( SDL_GetError() );
        //  return PJMEDIA_EVID_SYSERR;
        //}

        // ABCherniccccccccccccccccccccccccccccccc
        //xflag |= SDL_WINDOW_HIDDEN;

        //xflag |= SDL_WINDOW_OPENGL;

        //strm->Perwindow = SDL_ Create Window("", 0, 0, 1, 1, SDL_WINDOW_OPENGL| SDL_WINDOW_HIDDEN);
        //sprintf(strm->sBuf, "%p", strm->Perwindow);
        //SDL_SetHint(SDL_HINT_VIDEO_WINDOW_SHARE_PIXEL_FORMAT, strm->sBuf);
        //strm->window = SDL_ Create WindowFrom(hwnd->info.window);
        //SDL_SetHint(SDL_HINT_VIDEO_WINDOW_SHARE_PIXEL_FORMAT, NULL);
        //SDL_DestroyWindow(strm->Perwindow);  
        // ABCherniccccccccccccccccccccccccccccccc
    ...
    // ABChernic
    int sdlScreenW = strm->param.disp_size.w;
    int sdlScreenH = strm->param.disp_size.h;
    strm->pToolBar= new SDL_ToolBar(strm->renderer, sdlScreenW, sdlScreenH, 80, 80);
...
#endif /* PJMEDIA_VIDEO_DEV_SDL_HAS_OPENGL */
    {    
        strm->scr_texS = SDL_CreateTexture(strm->renderer, sdl_format,          // create     : sdl_create_window
                                          SDL_TEXTUREACCESS_STREAMING,
                                          strm->rect.w, strm->rect.h);
        if (strm->scr_texS == NULL) {                                           // create     : sdl_create_window
                               ...
                               pjmedia_rect_size *new_disp_size)
{
    ...
    if (strm->scr_texS) {                                                       // resize     : resize_disp
        strm->dstrect.x = strm->dstrect.y = 0;                                  
        strm->dstrect.w = (Uint16)strm->param.disp_size.w;                      
        strm->dstrect.h = (Uint16)strm->param.disp_size.h;                      
...
static pj_status_t put_frame(void *data)
{
    ...
    //ABChernic : MIX
    if (stream->scr_texS) {                                                     // put_frame
        SDL_UpdateTexture(stream->scr_texS, NULL, frame->buf, stream->pitch);   // put_frame
        SDL_RenderClear(stream->renderer);
        SDL_RenderCopy(stream->renderer, stream->scr_texS,                      // put_frame
