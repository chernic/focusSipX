...
PJ_DEF(pj_bool_t)pjmedia_vid_dev_stream_is_running(pjmedia_vid_dev_stream *strm){
    return strm->sys.is_running;
}

/* API: Get the running parameters for the specified video stream.  */  // O1 *get_param
PJ_DEF(pj_status_t) pjmedia_vid_dev_stream_get_param(pjmedia_vid_dev_stream *strm, pjmedia_vid_dev_param *param){
    pj_status_t status;
...
/* API: Get the value of a specific capability of the video stream. */  // O2 get_cap
PJ_DEF(pj_status_t) pjmedia_vid_dev_stream_get_cap(pjmedia_vid_dev_stream *strm, pjmedia_vid_dev_cap cap,void *value){
    return strm->op->get_cap(strm, cap, value);
}

/* API: Set the value of a specific capability of the video stream. */	// O3 *set_cap
PJ_DEF(pj_status_t) pjmedia_vid_dev_stream_set_cap(pjmedia_vid_dev_stream *strm,pjmedia_vid_dev_cap cap,const void *value){
...
/* API: Set the dir of bmp files of the video stream.				*/	// O9 *set_dir		
PJ_DEF(pj_status_t) pjmedia_vid_dev_stream_set_dir(pjmedia_vid_dev_stream *strm,const void *value){
    return strm->op->set_dir(strm, value);
}
/* API: Stop the stream. */												// O5 *get_frame
PJ_DEF(pj_status_t) pjmedia_vid_dev_stream_get_frame(pjmedia_vid_dev_stream *strm,pjmedia_frame *frame){
...
/* API: Stop the stream. */												// 10 *put_previw (via:typedef struct pjmedia_vid_dev_stream_op)
PJ_DEF(pj_status_t) pjmedia_vid_dev_previw_put_frame(pjmedia_vid_dev_stream *strm,const pjmedia_frame *frame){
    pj_assert(strm->op->put_previw);
    return strm->op->put_previw(strm, frame);
}
/* API: Stop the stream. */												// 11 *put_stream (via:typedef struct pjmedia_vid_dev_stream_op)
