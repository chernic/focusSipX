...
status = pjmedia_vid_stream_create(
    pjsua_var.med_endpt,
    NULL,
    si,
    call_med->tp,
...
/* Start stream */
status = pjmedia_vid_stream_start(
    call_med->strm.v.stream);
    if (status != PJ_SUCCESS){
        goto on_error;
    }
...
if (si->dir & PJMEDIA_DIR_DECODING)
{
    ...
    //S04 : Connect renderer to stream
    status = pjmedia_vid_port_connect(w->vp_rend,
        media_port,
        PJ_FALSE);
        if (status != PJ_SUCCESS) {
            pj_log_pop_indent();
    ...
    //S05 : Connect stream to capturer
    //
    //
    //
    //
